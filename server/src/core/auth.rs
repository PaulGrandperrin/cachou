use std::convert::TryFrom;

use anyhow::anyhow;
use chrono::Duration;
use common::api::{self, GetUserIdFromEmail, LoginFinish, LoginStart, SignupFinish, SignupStart};
use opaque_ke::{CredentialFinalization, CredentialRequest, RegistrationRequest, RegistrationUpload, ServerLogin, ServerLoginStartParameters, ServerRegistration, keypair::KeyPair};
use rand::Rng;
use tide::Request;
use tracing::{error, info, trace};
use common::crypto::OpaqueConf;
use async_trait::async_trait;

use crate::rpc::{RpcExec, RpcRetEncode};

#[async_trait]
impl RpcExec for GetUserIdFromEmail {
    async fn exec(&self, req: Request<crate::state::State>) -> anyhow::Result<Box<dyn RpcRetEncode>> {
        let user_id = req.state().db.get_user_id_from_email(&self.email).await?;

        Ok(Box::new(user_id))
    }
}


#[async_trait]
impl RpcExec for SignupStart {
    async fn exec(&self, req: Request<crate::state::State>) -> anyhow::Result<Box<dyn RpcRetEncode>> {
        let mut rng = rand_core::OsRng;
        let opaque = ServerRegistration::<OpaqueConf>::start(
            &mut rng,
            RegistrationRequest::deserialize(&self.opaque_msg)?,
            req.state().opaque_kp.public(),
        )?;
        let opaque_state = opaque.state.to_bytes();

        let user_id: [u8; 32] = rand::thread_rng().gen(); // 256bits, so I don't even have to think about birthday attacks
        let ip = req.peer_addr().map(|a|{a.split(':').next()}).flatten().ok_or_else(||{anyhow!("failed to determine client ip")})?;
        let expiration = (chrono::Utc::now() + Duration::minutes(1)).timestamp();
        
        req.state().db.save_opaque_state(&user_id, ip, expiration, &opaque_state).await?;

        Ok(Box::new((
            user_id.to_vec(),
            opaque.message.serialize()
        )))
    }
}

#[async_trait]
impl RpcExec for SignupFinish {
    async fn exec(&self, req: Request<crate::state::State>) -> anyhow::Result<Box<dyn RpcRetEncode>> {
        let opaque_state = req.state().db.restore_opaque_state(&self.user_id).await?;
        let opaque_state = ServerRegistration::<OpaqueConf>::try_from(&opaque_state[..])?;
    
        let opaque_password = opaque_state
            .finish(RegistrationUpload::deserialize(&self.opaque_msg)?)?;
    
        req.state().db.insert_user(&self.user_id, &self.email, &opaque_password.to_bytes()).await?;
    
        Ok(Box::new(()))
    }
}

#[async_trait]
impl RpcExec for LoginStart {
    async fn exec(&self, req: Request<crate::state::State>) -> anyhow::Result<Box<dyn RpcRetEncode>> {
        let mut rng = rand_core::OsRng;

        let opaque_password = req.state().db.get_opaque_password_from_user_id(&self.user_id).await?;
        
        let opaque_password = ServerRegistration::<OpaqueConf>::try_from(&opaque_password[..])?;
        let opaque = ServerLogin::start(
            &mut rng,
            opaque_password,
            req.state().opaque_kp.private(),
            CredentialRequest::deserialize(&self.opaque_msg)?,
            ServerLoginStartParameters::default(), // FIXME 
        )?;
    
        let ip = req.peer_addr().map(|a|{a.split(':').next()}).flatten().ok_or_else(||{anyhow!("failed to determine client ip")})?;
        let expiration = (chrono::Utc::now() + Duration::minutes(1)).timestamp();
        req.state().db.save_opaque_state(&self.user_id, ip, expiration, &opaque.state.to_bytes()).await?;
        
        let opaque_msg = opaque.message.serialize();
    
        Ok(Box::new(opaque_msg))
    }
}

#[async_trait]
impl RpcExec for LoginFinish {
    async fn exec(&self, req: Request<crate::state::State>) -> anyhow::Result<Box<dyn RpcRetEncode>> {
        let opaque_state = req.state().db.restore_opaque_state(&self.user_id).await?;
        let opaque_state = ServerLogin::<OpaqueConf>::try_from(&opaque_state[..])?;
        let _opaque_log_finish_result =opaque_state.finish(CredentialFinalization::deserialize(&self.opaque_msg)?)?;
    
        //opaque_log_finish_result.shared_secret
    
        Ok(Box::new(()))
    }
}




