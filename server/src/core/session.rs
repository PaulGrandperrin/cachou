use common::{api::{self, SealedSessionToken, session_token::{Clearance, SessionToken}}};

use crate::state::State;

impl State {
    pub fn session_token_new_need_second_factor_sealed(&self, user_id: Vec<u8>, version: u64) -> eyre::Result<SealedSessionToken> {
        SessionToken::new_need_second_factor(user_id, version).seal(&self.secret_key[..])
    }

    pub fn session_token_new_logged_in_sealed(&self, user_id: Vec<u8>, version: u64, auto_logout: bool, uber: bool) -> eyre::Result<SealedSessionToken> {
        SessionToken::new_logged_in(user_id, version, auto_logout, uber).seal(&self.secret_key[..])
    }

    pub async fn session_token_unseal_refreshed_and_validated(&self, sealed_session_token: &SealedSessionToken, required_clearance: Clearance) -> api::Result<SessionToken> {
    ...
    pub fn session_token_seal(&self, session_token: &SessionToken) -> eyre::Result<SealedSessionToken> {
