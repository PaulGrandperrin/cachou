    ...
    match resp.status() {
        StatusCode::OK => {
            ...
            for line in body.lines() {
                ...
                let line_hash = split_line.next().ok_or(eyre!("failed to parse HIBP response"))?;
                if line_hash == split_hash.1 {
                    let count = split_line.next().ok_or(eyre!("failed to parse HIBP response"))?;
                    return Ok(count.parse::<u64>().wrap_err("failed to parse HIBP response")?);
                }
            }
            Ok(0)
        },
